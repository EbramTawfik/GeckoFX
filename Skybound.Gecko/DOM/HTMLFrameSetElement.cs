...
{
	[Guid("a6cf90b8-15b3-11d2-932e-00805f8add32"), ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
	interface nsIDOMHTMLFrameSetElement : nsIDOMHTMLElement
	{
        ...
        #endregion
		void GetCols(nsAString aCols);
		void SetCols(nsAString aCols);

		void GetRows(nsAString aRows);
		void SetRows(nsAString aRows);
	...
	public class GeckoFrameSetElement : GeckoElement
	{
		...
		public string Cols {
			get { return nsString.Get(DOMHTMLElement.GetCols); }
			set { DOMHTMLElement.SetCols(new nsAString(value)); }
		}

		public string Rows {
			get { return nsString.Get(DOMHTMLElement.GetRows); }
			set { DOMHTMLElement.SetRows(new nsAString(value)); }
		}
